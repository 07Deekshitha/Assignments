...
if (creditScore > 750) {
    console.log(`line 40 Loan is automatically approved for ${customerName}`);
}
else if (creditScore >= 650 && creditScore <= 750) {
    if (income >= 50000) {
        if (isEmployed) {
            if (debtToIncomeRatio < 40) {
                console.log(`line 46 Loan is approved for ${customerName} as all the checks are qualified`);
            }
            else {
                console.log(`line 49 Loan is denied for ${customerName} due to high debt-to-income ratio`);
            }
        }
        else {
            console.log(`line 53 Loan is denied for ${customerName} due to unemployment`);
        }
    }
    else {
        console.log(`line 57 Loan is denied for ${customerName} due to insufficient income`);
    }
}
else {
    console.log(`line 61 Loan is denied for ${customerName} due to low credit score`);
    }